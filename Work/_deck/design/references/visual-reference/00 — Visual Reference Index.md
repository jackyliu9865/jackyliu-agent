...
The written rules in `design-principles.md` and `exhibits.md` describe this, and
describing it is not enough: a rule you read is not a page you have seen. **These 38
...
*Measured across all 38 pages: every exhibit in all three decks is **13.70 or
28.40 cm wide and nothing in between**, and every read column is 13.70 (or 9.30 on
a three-column page). Not one exhibit is 20.40, which is `TL_WIDE`.
`qa.gate_exhibit_width` enforces exactly those two widths, so any other number
fails the build.*
...
instruction, and the reason the rendered pages above show no terminal stops).
`ex.commentary()` adds it for you; `deckkit.bullets()` is for the
...
**Every colour on a page has to mean something.** Every non-default colour on a page encodes one stated thing, and the note or a legend
